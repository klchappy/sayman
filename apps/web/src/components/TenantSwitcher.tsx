...
export function TenantSwitcher() {
  ...
  return (
    <div data-tenant-switcher className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-3 px-3 py-1.5 rounded-lg border border-brand-200 hover:bg-brand-50"
      >
        ...
        <div className="absolute left-0 top-full mt-2 w-96 bg-white rounded-xl shadow-lg ring-1 ring-brand-100 z-50 p-2">
          ...
          {me?.organizations.map((org) => (
            <button
              key={org.id}
              onClick={() => {
                setActive({ orgSlug: org.slug, tenantSlug: null });
              }}
          ...
          {active.orgSlug && (
            <>
              <div className="px-3 py-2 mt-2 border-t border-brand-100 pt-3">
              ...
              <div className="max-h-72 overflow-y-auto">
                ...
                {tenantsQuery.data?.map((tenant) => (
                  <button
                    key={tenant.id}
                    onClick={() => {
                      setActive({ tenantSlug: tenant.slug });
                      setOpen(false);
                    }}
                    className={`w-full text-left flex items-center justify-between px-3 py-2 rounded-lg text-sm ${
                      active.tenantSlug === tenant.slug ? 'bg-brand-50' : 'hover:bg-brand-50'
                    }`}
                  >
                    <div>
